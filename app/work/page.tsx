		...
		<section className="w-full h-full | flex flex-col justify-center items-center">
			<div className="px-4 | max-w-7xl w-full h-3/4 | grid grid-cols-3 grid-rows-3 | gap-2">
				...
				{Object.values(campaigns).map((campaign, index) => (
					<Link key={index} className="relative group | w-full | overflow-hidden" href={`work/${index}`}>
						{/* Images */}
						<Image
						...
						{/* Hover text */}
						<p className="absolute -bottom-full transition-[bottom] group-hover:bottom-0 left-0 right-0 | bg-background bg-opacity-80 rounded | p-2 m-2 text-text text-xl font-bold">
							{campaign[0].alt}
						</p>
					</Link>
				))}
